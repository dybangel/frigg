...
mysleep:function(min,max){
var thismin=min*1000;
var thismax=max*1000;
var s=Math.round(Math.random()*(thismax-thismin))+thismin;
log("随机延迟："+s);
sleep(s);
},
swipeup:function(){
    h=device.height;
    w=device.width;
    ws=Math.round(Math.random()*(w *0.3-w * 0.8))+w *0.8;
    log("ws is:"+ws);
    swipe(ws,h * 0.8,ws,h * 0.2,500);  
...
restartapp:function() {
    ...
    try {
        ...
        toast("restartapp openstate is:"+openstate)
        if (openstate) {
            if ("layers" == apptype) {
...
//播放声音
playglobal:function(voicename){
    media.playMusic("./voice/global/" + voicename + ".mp3");
 ...
 play:function(subpath, appname) {
      ...
      //  thiscommon.play("global", "打开");
       // thiscommon.play("appname", appname);
        ...
        for(i=1;i<7;i++){
            log("aaa:"+i);
          //  sleep(2000);
            var result=className("android.widget.TextView").text(appname).exists();
            if(result){
            ...
            toast("滑动");
            swipe(800,400,200,400,200);
            sleep(1000);
        }
        //return false;
     

      //  log("launch :"+packagename);
       // launch(packagename);
       // launchApp("快手极速版");
        //var result = thiscommon.openpackage(packagename);
    ...
    for (var i = 0; i < Gapps.length; i++) {
        ...
        if (Gapps[i]['enable']) {
            ...
            if (Gjsonloadstate == "remote") {
                if (voiceplaynum == 0) {
                    toast("加载特征码");//this.play("global", "加载");
                    // play("global","本地");
    ...
    loadGapps:function() {
        ...
        var result = files.exists("./applist/localgapps.json")
        if (result) {
           // toast("当前以本地开关量为准");
            var str = files.read("./applist/localgapps.json");
            //   alert(str);
            Gapps = eval(str)
    ...
    openpackage : function(packagestr) {
        log("dakaiapp..."+packagestr);
        var result=launch(packagestr)
        //  activity = packagestr;
            ...
            if("vmos"==devicetype){
               ...
               return;
                       ...
                       try{
                                    ...
                                    sleep(1500);
                                  //  toast("this is clean .......");
                                    result=className("android.widget.TextView").desc("一键加速").exists();
                                    if(result){
                                    className("android.widget.TextView").desc("一键加速").findOnce().click();
                                    }else{
                                    toast("请安装lbe加速大师，否则无法优化前台应用")
                                    }
                                //     recents();//最近任务
                                // sleep(1500);
